...
namespace CityBuilderCore
{
    ...
    {
        [Tooltip("optional addon that is put on buildings when they are placed, THREE uses a BuildingAddonTransformer here to make buildings pop up in an animation, alternatively an addon can be in the BuildingBuilder.Added event(see BuildingPlayground3D)")]
        ...
        public T Add<T>(Vector3 position, Quaternion rotation, T prefab, Action<T> initializer = null, UndoRedoActions undoRedoActions = null) where T : Building
        {
            ...
            undoRedoActions?.Add(new BuildingAddition(building));
        ...
        public IBuilding Add(BuildingMetaData metaData, UndoRedoActions undoRedoActions = null)
        {
            var info = Dependencies.Get<IKeyedSet<BuildingInfo>>().GetObject(metaData.Key);
            if (info == null)
                return null;
