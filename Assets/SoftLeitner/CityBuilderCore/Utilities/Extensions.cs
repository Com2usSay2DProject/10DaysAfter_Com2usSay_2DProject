...
namespace CityBuilderCore
{
    public static class Extensions
    {
        ...
        /// <summary>
        /// adds items to the storage and creates an undoredo action if the actions is not null
        /// </summary>
        /// <param name="itemStorage">storage to add items to</param>
        /// <param name="items">the items to add</param>
        /// <param name="actions">actions container or null if there is no undoredo</param>
        /// <returns>remaining quantity that did not fit</returns>
        public static int AddItems(this ItemStorage itemStorage, ItemQuantity items, UndoRedoActions actions)
        {
            if (actions != null)
                actions.Add(UndoRedoGlobalItems.CreateAddition(items));
            return itemStorage.AddItems(items);
        }
        /// <summary>
        /// removes items from storage and creates an undoredo action if the actions is not null
        /// </summary>
        /// <param name="itemStorage">storage to remove items from</param>
        /// <param name="items">the items to remove</param>
        /// <param name="actions">actions container or null if there is no undoredo</param>
        /// <returns>remaining quantity not removed</returns>
        public static int RemoveItems(this ItemStorage itemStorage, ItemQuantity items, UndoRedoActions actions)
        {
            if (actions != null)
                actions.Add(UndoRedoGlobalItems.CreateRemoval(items));
            return itemStorage.RemoveItems(items);
        }
        ...
        /// <param name="actions">actions container or null if there is no undoredo</param>
        /// <returns>remaining quantity that did not fit</returns>
        public static int AddItems(this ItemStorage itemStorage, Item item, int quantity, UndoRedoActions actions)
        {
            if (actions != null)
                actions.Add(UndoRedoGlobalItems.CreateAddition(item, quantity));
            return itemStorage.AddItems(item, quantity);
        }
        /// <summary>
        /// removes items from storage and creates an undoredo action if the actions is not null
        /// </summary>
        ...
        /// <param name="quantity">how many items to remove</param>
        /// <param name="actions">actions container or null if there is no undoredo</param>
        /// <returns>remaining quantity not removed</returns>
        public static int RemoveItems(this ItemStorage itemStorage, Item item, int quantity, UndoRedoActions actions)
        {
            if (actions != null)
                actions.Add(UndoRedoGlobalItems.CreateRemoval(item, quantity));
            return itemStorage.RemoveItems(item, quantity);
        }
        ...
        public static void Remove(this IStructure structure, Vector2Int point, UndoRedoActions undoRedoActions)
        {
            structure.Remove(new Vector2Int[] { point }, undoRedoActions);
        }
        public static void Add(this IStructure structure, Vector2Int point, UndoRedoActions undoRedoActions)
        {
            structure.Add(new Vector2Int[] { point }, undoRedoActions);
        }
        ...
        public static BuildingMetaData GetMetaData(this IBuilding building)
        {
            if (building == null)
                return null;
            ...
            return new BuildingMetaData()
            {
                ...
                Key = building.Info.Key,
                Index = building.Index,
                Point = building.Point,
                Rotation = building.Rotation.State,
                Data = building.SaveData()
            };
        }

        public static bool Remove(this IStructureManager structureManager, Vector2Int point, int mask, bool decoratorsOnly, Action<IStructure> removing = null, UndoRedoActions undoRedoActions = null)
        {
            return structureManager.Remove(new Vector2Int[] { point }, mask, decoratorsOnly, removing, undoRedoActions).Any();
        }
