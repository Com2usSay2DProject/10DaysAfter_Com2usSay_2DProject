...
namespace CityBuilderCore
{
    ...
    public interface IStructure : IKeyed
    {
        ...
        /// <returns>the actual removed points</returns>
        ...
        public static void ReplacePoints(IStructure a, IStructure b, Vector2Int point, bool keepRandomization = true, bool keepVariant = true, UndoRedoActions undoRedoActions = null)
        {
            if (a == null || b == null)
            {
                if (a != null)
            ...
            if (keepRandomization)
            {
                if (a is StructureCollectionFloat collectionFloatA && b is StructureCollectionFloat collectionFloatB)
                {
                    ...
                    collectionFloatA.Remove(point, undoRedoActions);
                    collectionFloatB.Add(point, structure.transform.localPosition, structure.transform.localRotation, structure.transform.localScale, variantIndex, undoRedoActions: undoRedoActions);
                    return;
                }

                if (a is StructureTerrainTrees treesA && b is StructureTerrainTrees treesB)
                {
                    var tree = treesA.GetTree(point);

                    treesA.Remove(point, undoRedoActions);
                    treesB.Add(point, tree, undoRedoActions: undoRedoActions);
                    return;
                }

                if (a is StructureTerrainTreeVariants treeVariantsA && b is StructureTerrainTreeVariants treeVariantsB)
                {
                    var tree = treeVariantsA.GetTree(point);
                    ...
                    treeVariantsA.Remove(point, undoRedoActions);
                    treeVariantsB.Add(point, tree, variantIndex, undoRedoActions);
                    return;
                }
            }
            else
            {
                ...
                if (variantCountA > 1 && variantCountA == variantCountB)
                {
                    ...
                    if (a is StructureCollectionFloat aCollectionFloat)
                    {
                        ...
                        aCollectionFloat.Remove(point, undoRedoActions);
                    }
                    else if (a is StructureTerrainTreeVariants aTrees)
                    {
                        variantIndex = aTrees.GetVariantIndex(point);
                        aTrees.Remove(point, undoRedoActions);
                    }

                    if (b is StructureCollectionFloat bCollectionFloat)
                    {
                        bCollectionFloat.Add(point, variantIndex, undoRedoActions);
                    }
                    else if (b is StructureTerrainTreeVariants bTrees)
                    {
                        bTrees.Add(point, variantIndex, undoRedoActions);
                    }
                    return;
                }
            }

            a.Remove(point, undoRedoActions);
            b.Add(point, undoRedoActions);
        }
        /// <summary>
        ...
        /// <param name="keepRandomization">transfer over any size/color randomizations, only works when the structure are of the same type</param>
        /// <param name="keepVariant">transfer over the variant index</param>
        public static void ReplacePoints(IStructure a, IStructure b, IEnumerable<Vector2Int> points, bool keepRandomization = true, bool keepVariant = true, UndoRedoActions undoRedoActions = null)
        {
            if (a == null || b == null)
            {
                if (a != null)
            ...
            if (keepRandomization)
            {
                if (a is StructureCollectionFloat collectionFloatA && b is StructureCollectionFloat collectionFloatB)
                {
                    if (keepVariant && collectionFloatA.Variants.Length > 1 && collectionFloatA.Variants.Length == collectionFloatB.Variants.Length)
                    {
                        ...
                        collectionFloatA.Remove(points, undoRedoActions);
                        foreach (var structure in structures)
                        {
                            collectionFloatB.Add(structure.Item1, structure.Item2.transform.localPosition, structure.Item2.transform.localRotation, structure.Item2.transform.localScale, structure.Item3, undoRedoActions: undoRedoActions);
                        }
                    }
                    else
                    {
                        ...
                        foreach (var structure in structures)
                        {
                            collectionFloatB.Add(structure.Item1, structure.Item2.transform.localPosition, structure.Item2.transform.localRotation, structure.Item2.transform.localScale, undoRedoActions: undoRedoActions);
                        }
                    }
                    return;
                }
                ...
                if (a is StructureTerrainTrees treesA && b is StructureTerrainTrees treesB)
                {
                    var trees = points.Select(p => Tuple.Create(p, treesA.GetTree(p))).ToArray();

                    treesA.Remove(points, undoRedoActions);
                    foreach (var tree in trees)
                    {
                        treesB.Add(tree.Item1, tree.Item2, undoRedoActions: undoRedoActions);
                    }
                    return;
                }
                ...
                if (a is StructureTerrainTreeVariants treeVariantsA && b is StructureTerrainTreeVariants treeVariantsB)
                {
                    if (keepVariant && treeVariantsA.Variants.Length > 1 && treeVariantsA.Variants.Length == treeVariantsB.Variants.Length)
                    {
                        ...
                        {
                            var t = treeVariantsA.GetTree(p);
                            var i = treeVariantsA.GetVariantIndex(t);

                            return Tuple.Create(p, t, i);
                        }).ToArray();

                        treeVariantsA.Remove(points, undoRedoActions);
                        foreach (var tree in trees)
                        {
                            treeVariantsB.Add(tree.Item1, tree.Item2, tree.Item3, undoRedoActions);
                        }
                    }
                    else
                    {
                        var trees = points.Select(p => Tuple.Create(p, treeVariantsA.GetTree(p))).ToArray();

                        treeVariantsA.Remove(points, undoRedoActions);
                        foreach (var tree in trees)
                        {
                            treeVariantsB.Add(tree.Item1, tree.Item2, undoRedoActions: undoRedoActions);
                        }
                    }
                    return;
                }
            ...
            else
            {
                ...
                if (variantCountA > 1 && variantCountA == variantCountB)
                {
                    ...
                    if (a is StructureCollectionFloat aCollectionFloat)
                    {
                        ...
                        aCollectionFloat.Remove(points, undoRedoActions);
                    }
                    else if (a is StructureTerrainTreeVariants aTreeVariants)
                    {
                        ...
                        aTreeVariants.Remove(points, undoRedoActions);
                    }
                    ...
                    if (b is StructureCollectionFloat bCollectionFloat)
                    {
                        ...
                        {
                            bCollectionFloat.Add(pointIndex.Item1, pointIndex.Item2, undoRedoActions);
                        }
                    }
                    else if (b is StructureTerrainTreeVariants bTreeVariants)
                    {
                        foreach (var pointIndex in pointIndices)
                        {
                            bTreeVariants.Add(pointIndex.Item1, pointIndex.Item2, undoRedoActions);
                        }
                    }
                    return;
                }
            ...
            a.Remove(points, undoRedoActions);
            b.Add(points, undoRedoActions);
        }
    }
}