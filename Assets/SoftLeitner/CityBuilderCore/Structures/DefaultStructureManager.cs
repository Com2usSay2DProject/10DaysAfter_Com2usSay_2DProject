...
check out the test scene in CityBuilderCore.Tests/City/Movements/StructurePathDebugging")]
        ...
        public IEnumerable<Vector2Int> Remove(IEnumerable<Vector2Int> points, int mask, bool decoratorsOnly, Action<IStructure> removing = null, UndoRedoActions undoRedoActions = null)
        {
            ...
            foreach (var point in points)
            {
                foreach (var structure in GetStructures(point, mask, null, false, decoratorsOnly ? (bool?)true : null))
                {
                    if (!structure.IsDestructible)
                        continue;
                    if (decoratorsOnly && !structure.IsDecorator)
                        continue;
            ...
            //if no main structures(Buildings) are found add underlying structures(Roads)
            //this is done so buildings built on roads can be removed without the road
            ...
            foreach (var structure in structures)
            {
                ...
                foreach (var changedPoint in structure.Instance.Remove(points, undoRedoActions))
                {
                    if(!changedPoints.Contains(changedPoint))
