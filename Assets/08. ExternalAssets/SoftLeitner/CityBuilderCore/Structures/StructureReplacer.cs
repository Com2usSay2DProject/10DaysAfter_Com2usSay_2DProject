...
namespace CityBuilderCore
{
    ...
    public class StructureReplacer : PointerToolBase
    {
        ...
        protected override void updatePointer(Vector2Int mousePoint, Vector2Int dragStart, bool isDown, bool isApply)
        {
            ...
            if (isApply && validPositions.Any())
            {
                if (validPositions.Any())
                    onApplied();

                var undoRedoActions = UndoRedoActions.Create($"Replace {_structureA.GetName()} with {_structureB.GetName()} Structures");

                foreach (var items in Cost)
                {
                    _globalStorage.Items.RemoveItems(items.Item, items.Quantity * validPositions.Count, undoRedoActions);
                }

                IStructure.ReplacePoints(_structureA, _structureB, validPositions, KeepRandomization, KeepVariant, undoRedoActions);
